...
bool Edge<T>::operator<(Edge<T> e1) const {
    return this->getWeight() > e1.getWeight();
}
...
vector<Vertex<T>* > Graph<T>::calculatePrim() {
    this->dijkstraShortestPath(this->vertexSet[0]->info);
...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
	...
	for(Vertex<T>* vert : this->vertexSet) {
	    vert->disjset.push_back(vert);
	    for(Edge<T> &e : vert->adj) {
	        queue.push(e);
	    }
	}
