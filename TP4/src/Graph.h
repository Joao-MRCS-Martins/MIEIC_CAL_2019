...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
	...
	Vertex<T>* destin = findVertex(dest);
	if(source != NULL && destin != NULL) {
        source->addEdge(destin,w);
        return true;
	}
	return false;
}
...
bool Graph<T>::removeEdge(const T &sourc, const T &dest) {
	...
	Vertex<T>* destin = findVertex(dest);
	if(source != NULL && destin != NULL) {
        return source->removeEdgeTo(destin);
	}
	return false;
}
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    if(vert != NULL) {
	    typename vector<Vertex<T>*>::iterator iter = vertexSet.begin();
	    for(;iter != vertexSet.end();iter++) {
	        this->removeEdge((*iter)->info,in);
	    }
	    vertexSet.erase(remove(vertexSet.begin(),vertexSet.end(),vert));
	    return true;
	}
	return false;
}
